...
@endphp
@php
    $upvoteClass = $existingVote && $existingVote->vote_type === 'up' ? 'text-purple-700 dark:text-purple-500' : '';
    $downvoteClass = $existingVote && $existingVote->vote_type === 'down' ? 'text-purple-700 dark:text-purple-500' : '';
@endphp

<svg rpl="" fill="currentColor" class="cursor-pointer upvote-post-button {{ $upvoteClass }}"
    data-post-id="{{ $post->id }}" height="20" icon-name="upvote-outline" viewBox="0 0 20 20" width="20"
...
<svg rpl="" fill="currentColor" class="cursor-pointer downvote-post-button {{ $downvoteClass }}"
    data-post-id="{{ $post->id }}" height="20" icon-name="downvote-outline" viewBox="0 0 20 20" width="20"
    xmlns="http://www.w3.org/2000/svg">
    <path
