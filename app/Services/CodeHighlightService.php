...
class CodeHighlightService
{
    ...
    public static function formatResponseContent($content): String
    {
         $pattern = '/```(\w+)?\s+(.*?)```/s';
         return preg_replace_callback($pattern, function ($matches) {
             $language = strtolower($matches[1]);
             $langClass = in_array($language, self::$supportedLanguages) ? "language-" . $language : "language-plaintext";
             $code = htmlentities($matches[2]);
             return "<pre><code class='bg-white dark:bg-gray-800 $langClass'>$code</code></pre>";
         }, $content);
    }
}